...
namespace Exercise6
{
    public static class LinqTasks
    {
        ...
        {
            var methodSyntax =
                Emps.Where(e => e.Job == "Backend programmer");

            var querySyntax = from e in Emps where e.Job == "Backend programmer" select e;
        ...
        public static IEnumerable<object> Task11()
        {
            var groupedDepts = Emps
                ...
                .Where(g => g.name != null);
            IEnumerable<object> result = groupedDepts;
            return result;
        }
        ...
        {
            IEnumerable<Emp> result = Emps.Where(e => Emps.Any(emp => emp.Mgr == e))
                .OrderBy(e => e.Ename)
        ...
        {
            var deptCounts = Emps.GroupBy(e => e.Deptno)
                .Select(g => new { Deptno = g.Key, Count = g.Count() });
            IEnumerable<Dept> result = Depts
                .GroupJoin(deptCounts,
                    d => d.Deptno,
                    dc => dc.Deptno,
                    (d, dc) => new { Dept = d, Count = dc.FirstOrDefault()?.Count ?? 0 })
